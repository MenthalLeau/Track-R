...
import { Search, User, Gamepad2, Trophy, Settings, LogOut, LogIn, Menu, X } from 'lucide-react';
...
export function Sidebar({ theme = 'dark' }: SidebarProps) {
    ...
    // 2. Gestion du clic extérieur (Profil)
    useEffect(() => {
    ...
    return (
        <>
            <button
                ...
                className={`lg:hidden fixed top-4 left-4 z-50 p-2 rounded-xl border shadow-lg transition-colors ${t.card.shadow} ${t.layout.border} ${t.text.main}`}
            ...
            {/* Sidebar Principale */}
            <aside className={`
                w-64 fixed inset-y-0 left-0 z-40 flex flex-col shadow-xl 
                transition-transform duration-300 ease-in-out border-r 
                ${t.layout.bg} ${t.layout.border}
                ${isMobileOpen ? 'translate-x-0' : '-translate-x-full'} 
                lg:translate-x-0
            `}>

                {/* En-tête : Logo */}
                <div className={`px-6 py-6 border-b transition-colors duration-300 ${t.layout.border} flex items-center justify-between`}>
                    ...
                    {/* Bouton fermer optionnel à l'intérieur du menu (pour mobile) */}
                ...
                <div className={`px-4 py-4 border-t transition-colors duration-300 ${t.layout.border} relative`} ref={menuRef}>
                    ...
                    {user ? (
                        <>
                            ...
                            >
                                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-purple-500 to-purple-600 flex items-center justify-center shadow-lg shadow-purple-500/30 overflow-hidden shrink-0">
                                    {avatarUrl ? (
